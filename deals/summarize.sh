#!/bin/bash
# Deals 대시보드의 [요약] 큐를 처리하는 로컬 워커.
#
# 흐름:
#   1) https://adb1.vercel.app/api/config → config.deals.pending 배열 읽기
#   2) deals.json 에서 대상 URL 들에 "요약대상": "Y" 마킹 (진행 전)
#   3) 각 URL 에 대해 claude -p "..." 실행 (/tech_blog 스킬 규칙 적용)
#   4) 성공한 URL 은 deals.json 에 "요약완료": "Y" + "요약완료_at" 마킹
#   5) KV pending 제거
#   6) deals.json 을 adb1 repo 로 push
set -uo pipefail
export PYTHONIOENCODING=utf-8

ADB1_REPO="C:/Code_Local/GitHub/adb1"
DEALS_JSON="$ADB1_REPO/deals/data/deals.json"
...
deals_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')
...
for url in pending:
    ...
    source = art.get('source') or '딜'
    title = art.get('title') or ''
    posted = art.get('posted') or today
    yymmdd = posted.replace('-', '')[2:8]

    prompt = (
        f"아래 VC/투자 딜 아티클에 /tech_blog 스킬의 요약·번역 규칙(한국어 3-bullet 요약 + 본문 한글 완역)을 "
...
if [ "$PROCESSED_COUNT" = "0" ]; then
    log "=== deals_summarize done (no changes) ==="
    exit 0
fi
...
if git diff --quiet -- deals/data/deals.json 2>/dev/null; then
    ...
    git -c user.name=timtsroh -c user.email=254851998+timtsroh@users.noreply.github.com \
        commit -m "chore(deals): mark ${PROCESSED_COUNT} deals summarized $(date +%Y-%m-%d)" \
        --author="timtsroh <254851998+timtsroh@users.noreply.github.com>" \
        2>&1 | tee -a "$LOG"
    git push origin main 2>&1 | tee -a "$LOG"
fi

log "=== deals_summarize done (processed=${PROCESSED_COUNT}) ==="
exit 0
